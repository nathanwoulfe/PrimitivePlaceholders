(() => {
    ...
    function primitiveHelperService() {
        ...
        const clampColor = x => Math.max(0, Math.min(255, x));

        const distanceToDifference = (distance, pixels) => Math.pow(distance * 255, 2) * (3 * pixels);

        const differenceToDistance = (diff, pixels) => Math.sqrt(diff / (3 * pixels)) / 255;

        const getScale = (width, height, limit) => Math.max(width / limit, height / limit, 1);

        const computeColor = (offset, imageData, alpha) => {
        ...
        const computeDifferenceChange = (offset, imageData, color) => {
        ...
        const computeColorAndDifferenceChange = (offset, imageData, alpha) => {
